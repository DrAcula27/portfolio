<h1 align="center">Welcome to my Portfolio!</h1>

> Originally created for my Software Engineering course with Per Scholas, I keep this portfolio up to date with my current information and my latest and greatest projects!

## Live Link

[https://danielleandrews-portfolio.onrender.com](https://danielleandrews-portfolio.onrender.com)

## Author

👤 **Danielle Andrews**

- Github: [@DrAcula27](https://github.com/DrAcula27)
- LinkedIn: [@daniellerandrews](https://linkedin.com/in/daniellerandrews)

## Technologies Used

My portfolio is bootstrapped with `create-react-app` and uses the following technologies:

- <img src="https://raw.githubusercontent.com/facebook/react/main/fixtures/attribute-behavior/public/favicon.ico" width="20" height="20" /> [React](https://reactjs.org/)
- <img src="https://raw.githubusercontent.com/nodejs/nodejs.org/main/static/images/logos/js-green.svg" width="20" height="20" /> [Node](https://nodejs.org/en/)

## Attributions

- Tree graphic is from [Clker-Free-Vector-Images on pixabay.com](https://pixabay.com/vectors/plant-silhouette-grey-tree-leaves-312737/?download).
- Acorn graphic is from [Clker-Free-Vector-Images on pixabay.com](https://pixabay.com/vectors/acorn-nut-oak-tree-autumn-seed-42791/).
